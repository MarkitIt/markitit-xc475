...
function parseEventDate(date: string): { 
  ...
  try {
    ...
    const simpleDateRegex = /([A-Za-z]+ \d{1,2}), (\d{1,2}:\d{2} [APap][Mm]) - ([A-Za-z]+ \d{1,2}), (\d{1,2}:\d{2} [APap][Mm])/;
    ...
    if (simpleDateMatch) {
      ...
      try {
        ...
        startDate = DateTime.fromFormat(`${simpleDateMatch[1]} ${currentYear} ${simpleDateMatch[2]}`, 'MMM d yyyy h:mm a', { zone: timezone });
        endDate = DateTime.fromFormat(`${simpleDateMatch[3]} ${currentYear} ${simpleDateMatch[4]}`, 'MMM d yyyy h:mm a', { zone: timezone });
        ...
        if (!startDate.isValid || !endDate.isValid) {
          ...
          if (!startDate.isValid) {
            logger.error('Start date error:', startDate.invalidReason, startDate.invalidExplanation);
          }
          if (!endDate.isValid) {
            logger.error('End date error:', endDate.invalidReason, endDate.invalidExplanation);
          }
        } else {
    ...
    // Check for Eventeny GMT format first (most specific and most common in the dataset)
      ...
      if (eventenyMatch) {
        ...
        try {
          ...
          // Parse dates with timezone
          startDate = DateTime.fromFormat(`${eventenyMatch[1]} ${eventenyMatch[2]}`, 'MMMM d, yyyy h:mm a', { zone: timezone });
          endDate = DateTime.fromFormat(`${eventenyMatch[3]} ${eventenyMatch[4]}`, 'MMMM d, yyyy h:mm a', { zone: timezone });
          
          // Try alternate format if first attempt fails
          if (!startDate.isValid) {
            startDate = DateTime.fromFormat(`${eventenyMatch[1]} ${eventenyMatch[2]}`, 'MMM d, yyyy h:mm a', { zone: timezone });
          }
          
          if (!endDate.isValid) {
            endDate = DateTime.fromFormat(`${eventenyMatch[3]} ${eventenyMatch[4]}`, 'MMM d, yyyy h:mm a', { zone: timezone });
          }
        } catch (err) {
          const error = err as Error;
      ...
      // EventBrite multi-day format
        ...
        if (eventBriteMatch) {
          logger.info("Matched eventBriteMultiDayRegex:", { match: eventBriteMatch });
          ...
          try {
            ...
            const error = err as Error;
            logger.error(`Error parsing EventBrite multi-day format: ${error.message}`);
          }
        }
        // Weekday with AM/PM time range
        else {
          const weekdayAmPmMatch = normalizedDate.match(weekdayAmPmTimeRangeRegex);
          if (weekdayAmPmMatch) {
            logger.info("Matched weekdayAmPmTimeRangeRegex:", { match: weekdayAmPmMatch });
            ...
            try {
              ...
              const error = err as Error;
              logger.error(`Error parsing weekday AM/PM format: ${error.message}`);
            }
          }
          // Weekday with time (only end has AM/PM)
          else {
            const weekdayTimeMatch = normalizedDate.match(weekdayTimeRangeRegex);
            if (weekdayTimeMatch) {
              logger.info("Matched weekdayTimeRangeRegex:", { match: weekdayTimeMatch });
              ...
              try {
                ...
                const error = err as Error;
                logger.error(`Error parsing weekday time range format: ${error.message}`);
              }
            }
            // Weekday with doors info
            else {
              const weekdayDoorsMatch = normalizedDate.match(weekdayWithDoorsRegex);
              if (weekdayDoorsMatch) {
                logger.info("Matched weekdayWithDoorsRegex:", { match: weekdayDoorsMatch });
                ...
                try {
                  ...
                  const error = err as Error;
                  logger.error(`Error parsing weekday with doors format: ${error.message}`);
                }
              }
              // Zapplication format
              else {
                const zapplicationMatch = normalizedDate.match(zapplicationRegex);
                if (zapplicationMatch) {
                  logger.info("Matched zapplicationRegex:", { match: zapplicationMatch });
                  
                  try {
                    // Zapplication typically uses MM/DD/YY format
                    // For dates, we'll use noon (12:00 PM) as start and 11:59 PM as end time
                    startDate = DateTime.fromFormat(zapplicationMatch[1], 'M/d/yy', { zone: 'local' }).set({ hour: 12 });
                    
                    // For end date, use 11:59 PM of the end date
                    endDate = DateTime.fromFormat(zapplicationMatch[2], 'M/d/yy', { zone: 'local' }).set({ hour: 23, minute: 59 });
                  } catch (err) {
                    const error = err as Error;
                    logger.error(`Error parsing Zapplication format: ${error.message}`);
                  }
                }
                else {
                  logger.info(`Could not match any regex pattern for: "${normalizedDate}"`);
                }
              }
            }
          }
    ...
    if (startDate && !startDate.isValid) {
      logger.info(`Invalid start date parsed: ${startDate.invalidReason} - ${startDate.invalidExplanation}`);
      startDate = null;
    }
    
    if (endDate && !endDate.isValid) {
      logger.info(`Invalid end date parsed: ${endDate.invalidReason} - ${endDate.invalidExplanation}`);
      endDate = null;
    }
  ...
  return {
    startDate: startDate ? admin.firestore.Timestamp.fromDate(startDate.toJSDate()) : null,
    endDate: endDate ? admin.firestore.Timestamp.fromDate(endDate.toJSDate()) : null,
  };
}
...
export const updateAllEventDates = onSchedule({
  ...
  try {
    ...
    logger.info(`Completed processing all events. Updated ${updatedCount} events`);
...
export const processAllEvents = onRequest({
  ...
  try {
    ...
    for (const doc of eventsSnapshot.docs) {
      ...
      const hasValidStartDate = eventData.startDate && eventData.startDate.seconds !== 0;
      const hasValidEndDate = eventData.endDate && eventData.endDate.seconds !== 0;
      
      // Process if we have a date and either timestamp is missing or zero
      if (dateToProcess && (!hasValidStartDate || !hasValidEndDate)) {
        logger.info(`Processing event ${doc.id} with date: "${dateToProcess}"`);
        ...
        if (Object.keys(updateData).length > 0) {
          ...
          logger.warn(`Could not parse date for event ${doc.id}: "${dateToProcess}"`);
        }
      } else {
        if (!dateToProcess) {
          logger.info(`Skipping event ${doc.id}: no date or detailed_date field`);
        } else {
          logger.info(`Skipping event ${doc.id}: already has valid start/end dates`);
        }
      }
