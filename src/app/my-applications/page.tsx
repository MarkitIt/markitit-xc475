...
export default function MyApplicationsPage() {
  ...
  useEffect(() => {
    ...
    const checkFirestore = async () => {
      try {
        ...
        console.log(
          "FIRESTORE DEBUG - Database connection successful, found users:",
          snapshot.size,
        );
      } catch (error) {
        console.error(
          "FIRESTORE DEBUG - Error connecting to Firestore:",
          error,
        );
      }
    };
  ...
  useEffect(() => {
    const fetchApplications = async () => {
      ...
      try {
        console.log(
          "FETCH DEBUG - Attempting to fetch applications for UID:",
          user.uid,
        );
        ...
        if (userDocSnap.exists()) {
          ...
          const applicationsWithStatus = await Promise.all(
            events.map(async (event: any) => {
              ...
              console.log("FETCH DEBUG - Processing event:", {
                eventId,
                eventName,
              });
              ...
              if (vendorApplyDocSnap.exists()) {
                ...
                console.log(
                  "FETCH DEBUG - Found vendorApply document:",
                  vendorApplyData,
                );
                ...
                return {
                  eventId,
                  eventName:
                    eventName || vendorApplyData.eventName || "Unnamed Event", // Use eventName from the user's document or fallback to vendorApply
                ...
                console.log(
                  "FETCH DEBUG - VendorApply document not found for event:",
                  eventId,
                );
                return {
                  eventId,
          ...
          if (user.email) {
            console.log(
              "FETCH DEBUG - Attempting to find user by email:",
              user.email,
            );
