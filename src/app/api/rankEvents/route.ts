...
const calculateCategoryScore = (vendor: any, event: any): number => {
  ...
  if (event.categories && vendor.categories) {
    if (event.categories.includes("All")) {
      score += 20;
    } else if (vendor.categories.some((category: string) => event.categories.includes(category))) {
      score += 20;
    }
  }
...
const getWeekdayFromTimestamp = (timestamp: { seconds: number; nanoseconds: number }): string => {
...
if (vendor.eventPreference?.length > 0 && event.type?.length > 0) {
  let matchedTypes = vendor.eventPreference.filter((type: string) => event.type.includes(type)).length;
...
if (event.location?.city && event.location?.state) {
  eventCoordinates = await getLatLongFromCityState(event.location.city, event.location.state) || { lat: 0, lng: 0 };
}
...
if (
  ...
  if (vendor.preferredEventSize?.min && vendor.preferredEventSize?.max && event.headcount) {
...
if (event.startDate && event.endDate && Array.isArray(vendor.schedule?.preferredDays)) {
  ...
  const normalizedPreferredDays = vendor.schedule.preferredDays.map((day: string) => day.toLowerCase());
  const normalizedEventDays = Array.from(eventDays).map((day: string) => day.toLowerCase());
  ...
  const matches = normalizedPreferredDays.filter((day: string) => normalizedEventDays.includes(day)).length;
    ...
    for (const vendorDay of normalizedPreferredDays) {
      for (const eventDay of normalizedEventDays) {
        if (eventDay.includes(vendorDay) || vendorDay.includes(eventDay)) {
          console.log(`Partial match found: ${vendorDay} matches with ${eventDay}`);
        }
      }
    }
  }
  ...
  breakdown.total = Object.values(breakdown).reduce((sum, score) => 
    score === breakdown.total ? sum : sum + score, 0);
...
export async function POST(request: Request) {
  try {
    ...
    const vendorDoc = await getDocs(query(vendorRef, where('uid', '==', vendorId)));
    ...
    return NextResponse.json({ error: 'Failed to rank events' }, { status: 500 });
  }
}

export async function GET() {
  return NextResponse.json({ message: 'Use POST method to rank events' }, { status: 405 });
}