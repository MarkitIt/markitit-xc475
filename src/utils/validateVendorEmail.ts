...
export async function validateVendorEmail(user: User | null): Promise<{
  ...
  try {
    ...
    if (response.profileEmail && response.authEmail) {
      response.emailsMatch =
        response.authEmail.toLowerCase() ===
        response.profileEmail.toLowerCase();
    ...
    if (!response.emailsMatch) {
      ...
      if (!emailSnapshot.empty) {
        response.errorMessage =
          "This email is associated with a different vendor profile";
      } else {
        response.errorMessage =
          "Vendor profile email does not match your account email";
      }
    }
