...
export async function getLatLongFromCityState(city: string, state: string): Promise<{ lat: number; lng: number } | null> {
    if (!apiKey) {
    ...
    try {
        ...
        if (data.status === 'OK' && data.results.length > 0) {
            ...
            console.error('Error fetching coordinates:', data.status, data.error_message || '');
            return null;
        }
    } catch (error) {
...
export async function getCityState(latitude: number, longitude: number): Promise<{ city: string; state: string } | null> {
    if (!apiKey) {
        console.error('Google Maps API key not found');
        return null;
    }
    ...
    if (latitude > 24 && latitude < 50 && longitude > 65 && longitude < 125) {
        lng = -longitude;
        console.log('Fixing US longitude from positive to negative:', {original: longitude, fixed: lng});
    }
    ...
    try {
        ...
        if (data.status === 'OK' && data.results?.length > 0) {
            ...
            console.log('Final extracted location:', { city, state, formattedAddress: result.formatted_address });
            return { city, state };
        }
...
export function parseEventDate(detailedDate: string): {
  ...
  const fullDateRegex = /([A-Za-z]+ \d{1,2}, \d{4}) \u00B7 (\d{1,2}:\d{2} [APap][Mm]) - ([A-Za-z]+ \d{1,2}, \d{4}) \u00B7 (\d{1,2}:\d{2} [APap][Mm])/;
  const singleDateRegex = /([A-Za-z]+, [A-Za-z]+ \d{1,2}) \u00B7 (\d{1,2}(?::\d{2})? ?[APap][Mm]) - (\d{1,2}(?::\d{2})? ?[APap][Mm]) (\w{3,4})/;
  ...
  if (fullDateRegex.test(detailedDate)) {
    ...
    if (match) {
      startDate = DateTime.fromFormat(`${match[1]} ${match[2]}`, 'MMMM d, yyyy h:mm a', { zone: 'local' });
      endDate = DateTime.fromFormat(`${match[3]} ${match[4]}`, 'MMMM d, yyyy h:mm a', { zone: 'local' });
    }
  } else if (singleDateRegex.test(detailedDate)) {
    const match = detailedDate.match(singleDateRegex);
    if (match) {
      startDate = DateTime.fromFormat(`${match[1]} ${currentYear} ${match[2]}`, 'EEEE, MMMM d yyyy h:mm a', { zone: match[4] });
      endDate = DateTime.fromFormat(`${match[1]} ${currentYear} ${match[3]}`, 'EEEE, MMMM d yyyy h:mm a', { zone: match[4] });
    }
  }

  function toTimestamp(dateTime: DateTime | null) {
    return dateTime ? { seconds: Math.floor(dateTime.toSeconds()), nanoseconds: 0 } : undefined;
  }
