...
try {
  ...
  console.error(
    "Make sure your service account file is correctly placed at:",
    serviceAccountPath,
  );
  process.exit(1);
}
  ...
  try {
    ...
    // EventBrite multi-day format
      ...
      try {
        ...
        console.error(
          `Error parsing EventBrite multi-day format: ${err.message}`,
        );
      }
    }
    ...
    // Weekday with time (only end has AM/PM)
      ...
      try {
        ...
        console.error(
          `Error parsing weekday time range format: ${err.message}`,
        );
      }
    }
    // Weekday with doors info
      ...
      try {
        ...
        console.error(
          `Error parsing weekday with doors format: ${err.message}`,
        );
      }
    }
    // Zapplication format
      ...
      try {
        ...
        startDate = DateTime.fromFormat(match[1], "M/d/yy", {
          zone: "local",
        }).set({ hour: 12 });

        // For end date, use 11:59 PM of the end date
        endDate = DateTime.fromFormat(match[2], "M/d/yy", {
          zone: "local",
...
async function migrateExistingEvents() {
  try {
    ...
    if (forceMode) {
      console.log(
        "FORCE MODE ENABLED: Will update all events with detailed_date, even if they already have timestamps",
      );
    }
    ...
    for (let i = 0; i < Math.min(5, eventsSnapshot.docs.length); i++) {
      ...
      console.log(
        `Event ${i + 1}: ID=${sampleDoc.id}, detailed_date="${sampleData.detailed_date || "NOT SET"}"`,
      );
      if (sampleData.startDate) {
        console.log(
          `  Already has startDate: ${sampleData.startDate.toDate().toISOString()}`,
        );
      }
      if (sampleData.endDate) {
        console.log(
          `  Already has endDate: ${sampleData.endDate.toDate().toISOString()}`,
        );
      }
    }
    ...
    for (const doc of eventsSnapshot.docs) {
      ...
      try {
        // Parse the dates
        console.log(
          `Processing event ${eventId} with detailed_date: "${eventData.detailed_date}"`,
        );
        ...
        if (!startDate && !endDate) {
          console.log(
            `Skipping event ${eventId}: couldn't parse dates from "${eventData.detailed_date}"`,
          );
          skipCount++;
          continue;
        }
        ...
        // Otherwise, compare with existing timestamps
          ...
          if (!eventData.startDate && startDate) {
            ...
            needsUpdate = true;
            console.log(
              `Updating startDate: ${eventData.startDate.toDate().toISOString()} -> ${startDate.toDate().toISOString()}`,
            );
          }
          ...
          if (!eventData.endDate && endDate) {
            ...
            needsUpdate = true;
            console.log(
              `Updating endDate: ${eventData.endDate.toDate().toISOString()} -> ${endDate.toDate().toISOString()}`,
            );
          }
          ...
          if (!needsUpdate) {
            console.log(
              `Skipping event ${eventId}: parsed timestamps match existing ones`,
            );
            skipCount++;
            parsedButSameCount++;
            continue;
        ...
        console.log(
          `Added event ${eventId} to batch with startDate: ${startDate?.toDate().toISOString()}, endDate: ${endDate?.toDate().toISOString()}`,
        );
    ...
    console.log(
      `  - Already has timestamps: ${skippedExistingTimestampsCount}`,
    );
    ...
    if (updateCount === 0) {
      ...
      console.log(
        "even if they already have timestamps, run with: npm run migrate -- --force",
      );
    }
  } catch (error) {
...
async function testDateParsing() {
  ...
  console.log(
    `- Successful parses: ${successCount} (${Math.round((successCount / testCases.length) * 100)}%)`,
  );
  console.log(
    `- Partial parses: ${partialCount} (${Math.round((partialCount / testCases.length) * 100)}%)`,
  );
  console.log(
    `- Failed parses: ${failCount} (${Math.round((failCount / testCases.length) * 100)}%)`,
  );
...
async function main() {
  ...
  if (isTestMode) {
    ...
    console.log(
      "Test complete. To run the actual migration, use: npm run migrate",
    );
    process.exit(0);
  } else {
